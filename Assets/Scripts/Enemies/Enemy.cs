...
public class Enemy : MonoBehaviour, IDamageable, IBounceable
{
    [Header("Enemy Stats")]
    ...
    void FixedUpdate()
    {
        if (isDead || (_knockback != null && _knockback.IsKnockedBack)) return;
        
        enemyRb.linearVelocity = new Vector2(enemyMovement.x * enemySpeed, enemyRb.linearVelocity.y);
    }
    ...
    {
        if (collision.transform.CompareTag("EnemyDirectionChanger")) FlipEnemy();
    }
